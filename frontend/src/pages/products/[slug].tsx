        ...
        <div className="mt-6 max-w-2xl mx-auto sm:px-6 lg:max-w-7xl lg:px-8 lg:grid lg:grid-cols-3 lg:gap-x-8">
          ...
          <div className="hidden lg:grid lg:grid-cols-1 lg:gap-y-8">
            <div className="aspect-w-3 aspect-h-2 rounded-lg overflow-hidden">
              <img
                src={selectedProduct.images[1]?.src}
                alt={selectedProduct.images[1]?.alt}
                className="w-full h-full object-center object-cover"
              />
            </div>
            <div className="aspect-w-3 aspect-h-2 rounded-lg overflow-hidden">
              <img
                src={selectedProduct.images[2]?.src}
                alt={selectedProduct.images[2]?.alt}
                className="w-full h-full object-center object-cover"
              />
            </div>
          </div>
          <div className="aspect-w-4 aspect-h-5 sm:rounded-lg sm:overflow-hidden lg:aspect-w-3 lg:aspect-h-4">
            <img
              src={selectedProduct.images[3]?.src}
              alt={selectedProduct.images[3]?.alt}
              className="w-full h-full object-center object-cover"
            />
          </div>
        </div>
          ...
          <div className="mt-4 lg:mt-0 lg:row-span-3">
            ...
            <form className="mt-10">
              ...
              <div>
                ...
                {/* <RadioGroup value={selectedProduct.color} onChange={(e:any) => {
                    setProduct({
                        ...selectedProduct,
...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  let products = await response.json(); // type is IList[]
  // const paths = null;
  ...
  return {
    paths: products.data.map((_product: IProduct) => {
      return {
...
export async function getStaticProps({ params }) {
  ...
  // const res = await fetch(`http://localhost:1337/api/products?slug=${params.slug}`);
  // strapi.query('restaurant').find({ id: 1 });
  // const product = await res.json() as IProduct;
  const productlist = await res.json() as IProductList;
